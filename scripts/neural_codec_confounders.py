...
class MetaEnCodecWrapper(BaseCodec):
    ...
    def __init__(self, sr: int = 16000, bandwidth: float = 1.5):
        """
...
class AudioLMCodecWrapper(BaseCodec):
    ...
    def __init__(self, sr: int = 16000, num_tokens: int = 1024):
        """
...
class VALLECodecWrapper(BaseCodec):
    ...
    def __init__(self, sr: int = 16000, num_quantizers: int = 8, num_tokens: int = 512):
        """
...
class NeuralCodecConfounder:
    ...
    def __init__(self, sr: int = 16000, init_only: Optional[str] = None, device_type: str = 'gpu'):
        """
    ...
    def _init_dac(self):
        ...
        if self.sr in [16000, 16000]:
            model_name = "44khz" if self.sr == 16000 else "16khz"
...
def apply_neural_codec(audio: np.ndarray, codec_name: str, sr: int = 16000) -> np.ndarray:
    """
...
def get_available_codecs(sr: int = 16000, device_type: str = 'gpu') -> List[str]:
